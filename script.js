$(function(){
    ...
    const $nextSlide = $("#next-page");
    const $currentImg = $("#page img");
    const $nextImg = $("#next-page img");
    const $currentH1 = $("#page h1");
    const $nextH1 = $("#next-page h1");
    const $titre = $("#titre");
    const $texte = $("#texte");
    ...
    function init() {
        $currentImg.attr("src", photos[0]);
        $currentH1.text(`${1} / ${len}`);
        $titre.text(titres[0]);
        $texte.text(textes[0]);
    ...
    function changeImage(nextIndex, direction) {
        ...
        $titre.fadeOut(200, () => {
            $titre.text(titres[nextIndex]).fadeIn(200);
        });
        
        $texte.fadeOut(200, () => {
            $texte.text(textes[nextIndex]).fadeIn(200);
        });
        
        $nextImg.attr("src", photos[nextIndex]);
        $nextH1.text(`${nextIndex + 1} / ${len}`);
        
        if (direction === 'next') {
            $currentSlide.css({
                animation: `slideOutLeft ${animationDuration}ms ease forwards`
            });
            
            $nextSlide.css({
                animation: `slideInRight ${animationDuration}ms ease forwards`,
                display: 'flex'
            });
        } else {
            $currentSlide.css({
                animation: `slideOutRight ${animationDuration}ms ease forwards`
            });
            
            $nextSlide.css({
                animation: `slideInLeft ${animationDuration}ms ease forwards`,
                display: 'flex'
            });
        }
        ...
        setTimeout(() => {
            ...
            $currentImg.attr("src", photos[nextIndex]);
            $currentH1.text(`${nextIndex + 1} / ${len}`);
    ...
    function adjustImageSize() {
        ...
        const $slides = $(".slide");
        const $slideImgs = $(".slide img");
        
        if (windowWidth >= 1200) {
            $slides.css({ width: "18.5em", height: "35em" });
            $slideImgs.css({ maxHeight: "32em" });
        } else if (windowWidth >= 1024) {
            $slides.css({ width: "17em", height: "32em" });
            $slideImgs.css({ maxHeight: "29em" });
        } else if (windowWidth >= 900) {
            $slides.css({ width: "16em", height: "30em" });
            $slideImgs.css({ maxHeight: "27em" });
        } else if (windowWidth >= 768) {
            $slides.css({ width: "15em", height: "28em" });
            $slideImgs.css({ maxHeight: "25em" });
        } else if (windowWidth >= 600) {
            $slides.css({ width: "14em", height: "26em" });
            $slideImgs.css({ maxHeight: "22em" });
        } else if (windowWidth >= 480) {
            $slides.css({ width: "12em", height: "22em" });
            $slideImgs.css({ maxHeight: "18em" });
        } else {
            $slides.css({ width: "10em", height: "18em" });
            $slideImgs.css({ maxHeight: "15em" });
        }
    }
